...
from app.services.ai_triage_service import AITriageService, SYMPTOM_LIST, SPECIALIST_MAP

# Let's generate a cleaner dataset
diseases = list(SPECIALIST_MAP.keys())
np.random.seed(42)
rows = []

from app.services.ai_triage_service import AITriageService
# Use the same map from AITriageService, but import it or define it:
# Let's read the map from app/services/ai_triage_service.py or recreate it here.
# Since we just want to test, let's write a python generator that runs with different settings.
...
disease_symptom_map = {
    ...
    'Jaundice': ['itching', 'vomiting', 'fatigue', 'weight_loss', 'high_fever', 'dark_urine', 'abdominal_pain'],
    ...
    '(vertigo) Paroxymal  Positional Vertigo': ['vomiting', 'headache', 'nausea', 'spinning_movements', 'loss_of_balance', 'unsteadiness'],
...
for disease, symptoms in disease_symptom_map.items():
    # 1. First generate 5 clean typical cases with 0 dropout and 0 noise
    for _ in range(5):
        ...
        rows.append(row)
    # 2. Then generate 25 cases with low dropout and low noise
    for _ in range(25):
        row = {s: 0 for s in SYMPTOM_LIST}
        for s in symptoms:
            if s in row:
        ...
        noise_symptoms = np.random.choice(SYMPTOM_LIST, noise_count, replace=False)
...
print("Training accuracy score (new params):", model.score(X, y_encoded))

# Test predict cold symptoms
features = np.zeros(len(symptom_cols))
cold_symptoms = ['continuous_sneezing', 'cough', 'runny_nose', 'congestion', 'throat_irritation']
for s in cold_symptoms:
    if s in symptom_cols:
        features[symptom_cols.index(s)] = 1
pred = model.predict([features])[0]
print("Prediction prognosis:", le.inverse_transform([pred])[0])
