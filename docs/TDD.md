...
| Notification Subsystem | Sends updates to patients and doctors via n8n workflows |
...
| `notification` | Integration with external notification workflow system (n8n) |
...
| AI Classification | Python with scikit-learn (Decision Tree), pandas, NumPy | Analyses patient symptoms to classify cases as urgent or non-urgent; provides specialist recommendations. Trained using publicly available healthcare dataset. |
| Workflow Automation | n8n (open-source, latest stable) | Automates workflows to notify about appointments, urgency notifications, and schedule updates. |
...
| Network | Local server (localhost) + API links between backend and n8n | Secure communication environment and automated workflow execution. |
...
| Workflow Automation Tool | n8n | Latest stable | Open-source |
...
| Version Control | GitHub | Code management, secure storage of project repository version control. |
| Workflow Automation | n8n | Used to automate workflows to notify about appointments, urgency notifications, and updates on schedules. |
...
sequenceDiagram
    ...
    NS->>N8N: Send notification workflow
    N8N-->>Patient: Deliver appointment confirmation notification
...
- Integrated through API links between the backend and n8n (local server communication).
...
| Deployment | Local Server (localhost), n8n | Install system for demonstration; allow automatic notification and schedule adjustments. |

## 9.3 Operations Phase Notes

- n8n workflows implemented to allow automated notifications and schedule updates.
