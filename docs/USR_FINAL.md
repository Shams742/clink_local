...
| Notification Subsystem | Sends automated updates to patients and doctors via internal simulated email and database workflow automation |
...
|  Internal Notification Engine     |<--------------------------------|  CLINK_DB              |
...
| Normal Flow(s) – NF | 1. System detects appointment or schedule change. 2. System generates notification and stores it in the database while simulating an email dispatch. 3. Patient receives updated notification. |
...
- External notification handling is performed through an automation workflow (Internal Notification Engine).
