...
| AI Classification | Python with scikit-learn (Decision Tree), pandas, NumPy | Symptom classification model; trained on publicly available healthcare dataset |
| Workflow Automation | n8n (open-source) | Automated notifications for appointments, urgency notifications, schedule updates |
...
| Network | Local server (localhost) with API links between backend and n8n | Secure communication and automated workflow execution |
